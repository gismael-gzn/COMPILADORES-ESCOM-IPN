...
class Automata:
	...
	accepting_states):
		...
		self._alphabet_ = alphabet
		self.__delta_map__ = Delta
		self._initial_state_ = initial_state
		self._accepting_states_ = accepting_states
	...
	def __ctrl_path__(self, sequence, pos, i, empty_flag):
		...
		if pos < len(sequence) and sequence[pos] in self._alphabet_:
			self.__last_valid_state__ = i
			self.__index__ = pos
		elif pos == len(sequence) and sequence[self.__index__] in self._alphabet_:
			self.__last_valid_state__ = i
		elif pos < len(sequence) and sequence[pos] not in self._alphabet_:
			self.__errset__.add(sequence[pos])

		if pos == len(sequence) and empty_flag == True:
			if self.__last_valid_state__ in self._accepting_states_:
	...
	def __initialize_processing__(self):
		self.__pathstack__ = []
		self.__errset__ = set([])
		self.__last_valid_state__ = self._initial_state_
		self.__pathstack__.append(self._initial_state_)
		self.__index__ = 0
	...
	def show_all_paths(self, sequence):
		if sequence not in self.__memo_accepted__:
			...
			self.__traverse_path__(self._initial_state_, sequence, 0)
